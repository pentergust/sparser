...
class UserIntentsStorage:
    ...
    def default(self) -> Intent:
        ...
        Данным методом мы получаем это намерение или пустое намерение.
        Данный метод испольузется, когда вам нужно передать в методы
        класса представления некоторое намерение.
        При таком сценарии в класс представления в любом случае
        попадёт намерение, даже если оно пустое.
    ...
    def get_default(self) -> Optional[Intent]:
