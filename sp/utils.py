...
def save_file(path: Path, data: dict | list):
    ...
    Deprecated:
        6.1: В скором времени проект откажется от использования json.
            Данные методы будут удалены.

    Args:
        path (Path): Путь к файлу для записи данных.
        data (dict | list): Данные для записи в файл.

    Returns:
        dict: Ваши данные для записи.
    """
...
def load_file(
    ...
    Args:
        ...
        data (dict | list | None): Данные для записи, по умолчанию не указаны.

    Returns:
        dict | list: Распакованные данные из файла.
    """
    try:
        with open(path) as f:
...
def plural_form(n: int, v: tuple[str, str, str]) -> str:
    ...
    Args:
        ...
        v (tuple[str, str, str]): Варианты слова (для 1, для 2, для 5).

    Returns:
        str: Склонённое слово в зависимости от числа.
    """
...
def get_str_timedelta(s: int, hours: bool | None=True) -> str:
    ...
    Args:
        ...
        hours (bool | None): Использовать ли часы, по умолчанию да.

    Returns:
        str: Строковый обратный отсчёт.
    """
    if hours:
        h, r = divmod(s, 3600)
        m, s = divmod(r, 60)
...
def compact_updates( # noqa
    ...
    **Правила совмещения**:
    ...
    Args:
        updates (list[dict[str, int | list[dict]]]):
            Список записей об обновлениях расписания.
