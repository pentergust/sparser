...
class TextCounter:
    ...
    Предварительно обработsdftn результат и собрав текстовые сообщения.
    ...
    def cl(self, intent: Intent, target: Optional[CounterTarget]=None) -> str:
        ...
        Использует намерения, для уточнения результатов подсчёта.
        Вы также можете выбрать отображаемую подргуппу.
        ...
        **Доступные подгруппы**:
        ...
        :type intent: Intent
        :param target: Отображаемая подгруппа.
    ...
    def days(self, intent: Intent, target: Optional[CounterTarget]=None) -> str:
        ...
        Группирует результаты и формарует текстовое сообщение.
        Использует намерения, для уточнения результатов подсчёта.
        Вы также можете выбрать отображаемую подгруппу.
