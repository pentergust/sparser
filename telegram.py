...
# Сообщение при выборе класса
SET_CLASS_MESSAGE = """
Для полноценной работы желательно указать ваш класс.
К примеру вы сможете быстро просматривать расписание и получать обновления.
...
# Сообщения интерактивного обучения запросам
TUTORIAL_MESSAGES = [
    # ------------------------------------------------------------------
    """💡 Как писать запросы?
...
Такие как "уроки на", "расписание", "для" и ещё много таких.
...
Порядок ключевых слов (класс, день, урок, кабинет) не имеет значение.
...
Вы можете более явно указаать день в запросах и поиске.
...
🔎 Укажите точное название урока, чтобы получить все его упоминания.
Если прочие аргументы не указаны, расписание для всех классов на неделю.

-- матем ➜ Вся математика на неделю для всех классов.
-- матем вторник 10а ➜ Более точный поиск.

⚠️ Если ввести несколько уроков, будет взять только первый.
Чтобы сообщение поиска не было слишком длинным.""",

    """[5/5] Поиск по кабинету
...
Чтобы сообщение поиска не было слишком длинным.
Однако можно указаать несколько предметов.""",
...
def get_tutorial_keyboard(i: int) -> InlineKeyboardMarkup:
    """Клавиатура многостраничного обучения.

    Испльзуется для перелистывания страниц обучения.

    Buttons:
        delete_msg => Удалить сообщение.
        tutorual:{i} => Сменить страницу обучения.

    Args:
        i (int): Текущая страница справки.
    ...
    if i == 0:
    ...
    elif i == len(TUTORIAL_MESSAGES)-1:
        inline_keyboard = [[
        ...
        inline_keyboard = [[
            InlineKeyboardButton(text="◁", callback_data=f"tutorial:{i-1}"),
            InlineKeyboardButton(text="🌟 Дальше", callback_data=f"tutorial:{i+1}")
        ],
