"""Предоставляет статические и динамеческие сообщения.

Эти самые сообщания можно использовать во всех обработчках бота.
"""
...
# Испольуется когда пользователь переходит в состояние смены класса
SET_CLASS_MESSAGE = (
...
# Содержит оснвовную информацию по использованию запросов
# Планируется заменить его на динамическое сообщение с виджетами
...
def get_intent_status(i: Intent) -> str:
    ...
    Обратите внимание, что дни недели помечаются цыфрами.
    0 - понедельник, 5 - суббота.

    :param i: Экземпляр намерения, содержимое которого нужно отобразить.
    :type i: Intent
...
def get_home_message(cl: str) -> str:
    ...
    По своей сути это сообщение дполняет само статическое главное
    сообщение, добавляя к нему описание текущего выбраного вами класса.

    Главное сообщение будет сопровождать пользователя на протяжении
    всего процесса использования бота.
    Суть главного сообщения - передваться основную необходимую
    ...
    :param cl: Для какого класса получаем главнео сообщения.
    :type cl: str
    :return: Дополненное главное сообщение бота.
    :rtype: str
    """
    if cl:
