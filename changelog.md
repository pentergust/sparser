...
Переписана большая часть кода, оптимизация, новые фишечки, упрощение чтения!
...
- Для ведения логов добавлено `loguru`.
- Данные парсера сохраняются в `sp_data/`.
- `save_file()`: Добавлено автоматическое создание родительских директорий.
- Полностью изменился формат хранения расписания, лучше читаемость, меньше места.
- Изменён формат ведения списка изменений.
- Едианя функция для получения `l_index` и `c_index` -> `get_index`.
- Индексы обновляются разово, вместе с расписанием.
- `ScheduleParser`: Стал независимым от пользователей и переимеован в `Schedule`.
- `_parse_schedule()`: Теперь сам определяет строки начало нового дня.
...
- `_process_update()`: Расписание обновляется точно через 3600 секунд, вместо проверки часа.
- `get_schedule()`: переименована в `get()`.

**SPMessages**:

- `send_status()`: Новый стиль, перечисление всхе доступных классов.
...
- `count_lessons()`: Отметка кабиентов/уроков, которые исепользуются единожды.
