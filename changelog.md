...
Масштабное обновление *Telegram* бота, связанное с частичной поддержкой
намерений (`sp.intents`).
...
- Добавлен новый класс `UserIntents()` для работы с намерениями пользовтелей:
...
- добавлены новые клавиатуры:
  - Клавиатура редактороа намерений.
  - Клавиатура редактирования отдельного намерения.
...
- Добавлены новые динамические сообщения:
...
- Новые обработчики:
  ...
  - Команда `/remove_intents`, кнопка `intents:remove_mode` => Режим удаления намерений.
  - Кнопка `intent:remove_many:{name}` => Удалить множественно намерение.
  - Кнопка `intents:remove_all` => Удалить все намерения.


## 27 Dec 2023: v5.3.8 -> 5.7

Список изменений Sp давно не обновлялся.
Чтобы навести порядок тут будут приведены все изменения начиная с
версии `sp v5.3.8` по актуальную версию `sp v5.7`.

- `v5.3.8`: Улучшено сообщение статуса.
...
- `v5.4`: Удалены устаревшие функции `search_lesson`, `search_cabinet`.
- `sp.spm` переименовано в `sp.messages`.
- Использоваие `DefaultDict` для функций счётчиков.
- `v5.5`: Класс `Filters` заменён на намерения (`Intents`).
  - Предоставляет методы для сборки: `construct()`, `parse()`.
  - Предоставляет методы дополнения: `reconstruct()`, `reparse()`.
  - Полная совместимость с фильтрами.
- Добавлён счётчик активных пользователей в сообщение статуса.
- `v5.7`: Изменён формат списка измененений.
...
- Улучшен заголовок списка изменений.
...
Появление нового интерасктивного обучения.
...
- Бот удаляет некоторые сообщения пользователя.


## 13 Dec 2023: tg v2.0

Полностью переписанная версия бота на модуле `aiogram v3.2`.
Общая чистка кода, подготовка к разделению на несколько файлов.
Исправления ошибок и некоторые небольшие улучшения.

Обнолвение **Telegram** до `v1.14`:

- Замена фильтров на намерения (Intents).
- Использование переменных игр вместо JSON файла `telegram.json`.
- Обновлены тексты сообщений.
- Добавлены вспомогательные кнопки при смене класса.
...
- Больше нет возможности передавать аргументы в команду.
- Больше нет возможности использовать бота в групповых чатах


## 10 Jul 2023: sp_vl v1.1 (sp v5.3.8)
