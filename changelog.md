...
- Метод `search()`: для поиска данных в расписании
...
- Изменены тексты сообщений для больше их ясности
...
- Убрана возможность переопределить пути хранения файлов.
- Больше не требует расписание как аргумент.
...
- `search_lesson()`: Переведено на использование `Filters`.
- `search_lesson()`: Изменение стиля сообщения.
- `search_cabinet()`: Переведено на использование `Filters`.
- `search_cabinet()`: Изменение стиля сообщения.

**Telegram**:

- Обновление до `sp v4.6`.
